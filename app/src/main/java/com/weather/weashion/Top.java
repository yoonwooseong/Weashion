...
public class Top {
    ...
    public Top(Context context) {
        ...
        randoms = Util.top[random];
        randoms2 = Util.top2[random2];
...
class NaverAsync extends AsyncTask<String, Void, ArrayList<ModelVO>>{
    ...
    protected ArrayList<ModelVO> doInBackground(String... strings) {
        list.add(parsers.Parser(start1,randoms2+" "+randoms));
        list.add(parsers.Parser(start2,randoms2+" "+randoms));
        list.add(parsers.Parser(start3,randoms2+" "+randoms));
        list.add(parsers.Parser(start4,randoms2+" "+randoms));
        return list;
    }
